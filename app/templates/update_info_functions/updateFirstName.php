<?php
//This page will offer the options of editing one's First Name.
...
$sqlname = $_SESSION['username'];
$fname = $fnameold = "";
$fnameErr = $fnameErr1 = $fnameErr2 = " ";

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    //fname validation
    if(empty($_POST["fname1"] || $_POST["fname2"])){
        $fnameErr= "First name is required ";
    } 
    else {
        $fnamevalidation = test_input($_POST["fname2"]);
        $fname = test_input($_POST["fname1"]);
        if (!preg_match("/^[a-zA-Z ]*$/",$fname) || !preg_match("/^[a-zA-Z ]*$/",$fnamevalidation)) {
            $fnameErr1 = "Only letters and white space allowed "; 
            }
            else{
            if($_POST["fname1"] !== test_input($_POST["fname2"])){
            $fnameErr2 = "Name inputs do not match! ";
            }
        }   
    }
}

if(isset($_POST["fname2"]) && isset($_POST["fname1"]) && $fnameErr === " " && $fnameErr1 === " " && $fnameErr2 === " "){
    $sql = "UPDATE users SET first_name = '$fname' WHERE first_name = '$sqlname' AND userid = '$sessionId'";
            ...
            <div class="col-sm-4">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    <div class="form-group">
                        <label for="lname"><b>Currently Used First Name:</b></label>
                        <input type="fnameold" class="form-control" name="fnameold" value="<?php echo $sqlname?>" disabled>
                    </div>
                    <div class="form-group">
                        <label for="fname1"><b>New First Name:</b></label>
                        <input type="text" class="form-control" name="fname1">
                        <span class="error"><?php echo $fnameErr . $fnameErr1;?></span>
                    </div>
                    <div class="form-group">
                        <label for="fname2"><b>Repeat New First Name:</b></label>
